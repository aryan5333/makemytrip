...
function AddEditHotel({ hotel, onSuccess }: { hotel: Hotel | null; onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage("Error: " + (error instanceof Error ? error.message : "Failed to save hotel"));
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {message && (
        <div className={`p-2 rounded ${message.startsWith("Error") ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
          {message}
        </div>
      )}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="hotels">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <HotelList key={refreshKey} onSelect={setSelectedHotel} />
                <AddEditHotel hotel={selectedHotel} onSuccess={() => {
